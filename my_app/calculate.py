...
def calculate_values(data=None):
        ...
        airbnb_service_charge_monthly =  float(days_occupied) * float(daily_rate) * airbnb_service_charge
        ...
        hotel_tax_reserve_monthly = (float(daily_rate) * float(hotel_tax_percentage)) * days_occupied

        # COSTS - totals
        utility_costs_monthly = float(water) + float(sewer) + float(garbage) + float(electric) + float(cable)

        service_costs_monthly = maid_costs_monthly + hotel_tax_reserve_monthly + airbnb_service_charge_monthly

        total_cost_monthly = float(utility_costs_monthly) + float(service_costs_monthly) + float(rent)
